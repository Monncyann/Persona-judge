...
class AutoModelForCausalLMWith2ValueHead(PreTrainedModelWrapper):
    r"""
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        self.v_head = ValueHead2(self.pretrained_model.config, **v_head_kwargs)
        self.w_head = ValueHead2(self.pretrained_model.config, **v_head_kwargs)
    ...
    def _init_weights(self, **kwargs):
        ...
        if init_strategy is None:
            ...
            self.w_head.summary.weight.data.normal_(mean=0.0, std=initializer_range)
            self.w_head.summary.bias.data.zero_()
    ...
    def forward(
        ...
        weights = self.w_head(last_hidden_state).squeeze(-1)
        value = torch.sum(values * weights, dim=-1)
