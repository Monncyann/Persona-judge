...
class ARGS:
    def __init__(self, llm_path, rm_path, llm_dev="cuda:0", rm_dev="cuda:1", torch_dtype=torch.float16):
        ...
        print("Loading LLM...")
        self.LLM = AutoModelForCausalLM.from_pretrained(llm_path, torch_dtype=torch_dtype, token= "").to(self.llm_dev)
        self.LLM.eval()
        ...
        self.RM.eval()
        self.RM.pad_token = self.tokenizer.eos_token
        self.RM.pad_token_id = self.tokenizer.pad_token_id
    ...
    def generate_step(self, mout, input_ids, pre_screen_beam_width=40, weight=0., method="greedy", temperature=0.7, rm_cached=None, debug=True):
        ...
        if rm_cached is None:
            rm_in = self.LLM.prepare_inputs_for_generation(input_ids=flat_trme.to(self.rm_dev), attention_mask=create_attention_mask(flat_trme.shape[1], flat_trme.shape[0]).to(self.rm_dev), past_key_values=None, use_cache=True)
            rm_in.pop("cache_position")
            print(rm_in)

            rm_out = self.RM(**rm_in)
            rm_cached = rm_out.past_key_values
        else:
            rm_in = self.LLM.prepare_inputs_for_generation(input_ids=flat_trme.to(self.rm_dev), attention_mask=create_attention_mask(flat_trme.shape[1], flat_trme.shape[0]).to(self.rm_dev), past_key_values=rm_cached, use_cache=True)
            rm_in.pop("cache_position")
            rm_out = self.RM(**rm_in)
            rm_cached = rm_out.past_key_values
