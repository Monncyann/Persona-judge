...
parser.add_argument("--rm", type=str, default="", help='The path of the reward model')
parser.add_argument("--tokenizer", type=str, default="", help='The path of the tokenizer')
parser.add_argument("--rm_gpu", type=str, default="cuda:2")
...
parser.add_argument("--end_index", type=int, help='End index for file pattern', default=7)
...
tokenizer = AutoTokenizer.from_pretrained(args.tokenizer, cache_dir="", token="")

num_labels = 2 if 'humor' in args.rm else 1

# fill in the parameter token and parameter cache_dir
if "ArmoRM" in args.rm:
    rm_model = AutoModelForSequenceClassification.from_pretrained(args.rm, trust_remote_code=True, torch_dtype=torch.bfloat16, token="").to(args.rm_gpu)
else:
    rm_model = AutoModelForSequenceClassification.from_pretrained(args.rm, num_labels=num_labels, torch_dtype=torch.float16, token="").to(args.rm_gpu)
...
def get_rm(text, dimension=None):
    if "ArmoRM" in args.rm or "PAD" in args.rm:
        ...
        # 1966 1819 1813
        if tokens.shape[1] >= 512: return None
